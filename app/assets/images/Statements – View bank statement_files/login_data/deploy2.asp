...
lpMTagConfig.plugins['simpleDeploy'] = {
    name: 'simpleDeploy',
    config: {
        // defaultButtonDivID : "lpChatButton",
        // sendOrderIfZeroOrEmpty : false,
        // scopeOverride : { "OrderTotal" : "page"},
        // varNameCharLimit : 50,
        // varValueCharLimit : 100,
        // SSO URL which will work for buttons and invites
        // SSOURL : "https://[someurl]"
        // SSO URL which will work for chat invites
        // inviteChatSSOurl : "https://[someurl]"
        // button objects can be configured to have an "SSOURL" property
        buttons: [{
            "name": "chat-" + lpMTagConfig.getLPVarValue("unit") + '-changeaddress-' + lpMTagConfig.getLPVarValue("language"),
            "pid": "LPButtonDiv-ChangeAddress",
            "afterStartPage": true,
            "ovr": "lpMTagConfig.db1"
        }, {
            "name": "chat-ulster-secure-ebanking-help-english",
            "pid": "ctl00_faqs_ctl00_LPButtonDivFAQWebchat",
            "afterStartPage": true,
            "ovr": "lpMTagConfig.db1"
        }, {
            "name": "chat-ulster-secure-ebanking-waferror-english",
            "pid": "ctl00_mainContent_lpButtonDiv-WAFError",
            "afterStartPage": true,
            "ovr": "lpMTagConfig.db1"
        }]
    }
...
if (unit == 'rbs-secure-ebanking' || unit == 'rbs-serviceFAQ') { //rbs
    BANKURL = 'https://www.rbsdigital.com/GetWebChatSSOKey.ashx';
    SetSSOURL(BANKURL);
} else if (unit == 'rbsInternational-secure-ebanking' || unit == 'rbsInternational-serviceFAQ') { //rbs international
    BANKURL = 'https://www.rbsidigital.com/GetWebChatSSOKey.ashx';
    SetSSOURL(BANKURL);
} else if (unit == 'ulster-secure-ebanking' || unit == 'ulster-serviceFAQ') { //ulster south
    BANKURL = 'https://www.ulsterbankanytimebanking.ie/GetWebChatSSOKey.ashx';
    SetSSOURL(BANKURL);
}

function SetSSOURL(BANKURL) {
    //console.log('URL passed to SSO Function: ' + BANKURL);
    var a = lpMTagConfig.plugins.simpleDeploy.config.buttons.length;
    //console.log('The number of buttons: ' + a);
    lpMTagConfig.SSOURL = BANKURL;
    for (i = 0; i < a; i++) lpMTagConfig.plugins.simpleDeploy.config.buttons[i].SSOURL = lpMTagConfig.SSOURL
};
